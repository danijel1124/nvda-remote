#!/usr/bin/env python3
"""Manually trigger an immediate server-update check ("check right now"),
without waiting for the running server's own scheduled check (see
server.py's _scheduled_update_check / update_check.py).
...
Check-only: never downloads or installs anything. See update_check.py's
...
def main():
	...
	result = update_check.check_for_update(server.SERVER_VERSION, args.data_dir, print)
